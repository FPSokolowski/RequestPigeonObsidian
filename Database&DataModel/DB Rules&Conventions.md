...
At SaveChanges intercept delete and add operations and if *ISoftDelete* implemented modify behavior (instead of delete just change *IsDeleted* property to *false*). Also deal with common properties declared in [[BaseEntity]]: at Add operation set *CreatedAt*, when Update  set new value to *LastModification*. 